...
public class TaskCreateActivity extends mAppCompatActivity{
    ...
    protected void taskList(Taskist task, int viewPage){
        ...
        if (!header.getText().toString().matches("")) {
            ...
            if (task == null) {
                task = Taskist.create(header.getText().toString());
    ...
    protected void saveList(){
        ...
        if (position >= 0) {
            Taskist tdl = App.Tasklists.get(position);
